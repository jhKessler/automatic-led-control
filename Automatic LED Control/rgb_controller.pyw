...
class ColorPicker(QColorDialog):
    ...
    # overwrite submit and cancel button functions
    def done(self, int):
...
class MainWindow(QMainWindow):
    ...
    def initUI(self, firstStart=True):
        ...
        # set up window when started for the first time
        if firstStart:
    ...
    def changeColor(self, colorObj):
        ...
        if rgb_value != (0, 0, 0):
            with open(r"C:\Users\Johnny\Desktop\Automatisierung\data\VALUES.txt", "w") as values_file:
                values_file.write(f"{red}:{green}:{blue}")
        else:
            with (r"C:\Users\Johnny\Desktop\Automatisierung\data\led_status.txt", "w") as led_status_file:
                led_status_file.write("off")
        
        # change led color
        requests.get(f"192.168.178.99/?red={red}&green{green}&blue={blue}")
